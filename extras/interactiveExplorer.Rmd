...
# Color by model origin (ensure consistent ordering)
originLevels <- c("Original Influenza", "New Covid")
devData$modelOrigin <- factor(devData$modelOrigin, levels = originLevels)
valData$modelOrigin <- factor(valData$modelOrigin, levels = originLevels)

# Two-color, colorblind-friendly palette by origin
originPalette <- c("#1f78b4", "#e31a1c")  # Influenza = blue, Covid = red
...
p <- plot_ly(source = "main_plot") |>
  ...
  add_segments(
    data = sharedVal,
    split = ~fullName, color = ~modelOrigin, colors = originPalette,
