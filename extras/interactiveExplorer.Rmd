...
expand_by_dev <- function(df, devLevels) {
  ...
  if (length(devLevels) > 0 && nrow(inf) > 0) {
    inf_exp <- tidyr::crossing(inf, devPeriod = devLevels)
    dplyr::bind_rows(
...
p <- plot_ly(source = "main_plot") |>
  ...
  add_segments(
    ...
    customdata = ~cbind(legendShort, populationSize, outcomeCount, startDateTime, endDateTime),
    hovertemplate = paste(
      "<b>%{customdata[0]}</b><br>",
      "AUROC: %{y:.3f}<br>",
      "Validation: %{customdata[3]|%Y-%m-%d} → %{customdata[4]|%Y-%m-%d}<br>",
      "N: %{customdata[1]:,}  |  Outcomes: %{customdata[2]:,}",
      "<extra></extra>"
    ),
    showlegend = TRUE
  ) |>
  ...
  add_segments(
    ...
    customdata = ~cbind(legendShort, devStartDateTime, devEndDateTime),
    hovertemplate = paste(
      "<b>%{customdata[0]}</b><br>",
      "AUROC: %{y:.3f}<br>",
      "Development: %{customdata[1]|%Y-%m-%d} → %{customdata[2]|%Y-%m-%d}",
      "<extra></extra>"
    ),
    showlegend = FALSE
  ) |>
